...
import { GridGroupHeader } from "./grid-group-header";
...
import { buildTableDisplayItems } from "@/ee/base/utils/table-grouping";
import {
...
type GridContainerProps = {
  ...
  /** Table-view data row height; group headers stay compact. */
  rowHeight?: string;
  /**
...
export function GridContainer({
  ...
  const displayItems = useMemo(
    ...
    [rows, groups, properties, collapsedGroupIds, editable],
  );
  const rowHeightKey = parseTableRowHeight(rowHeight);
  const dataRowHeightPx = rowHeightPx(rowHeightKey);
  const lineClamp = rowHeightLineClamp(rowHeightKey);
  const gridRootStyle = useMemo(
    () =>
      ({
        "--base-row-height": `${dataRowHeightPx}px`,
        "--base-compact-row-height": `${COMPACT_ROW_HEIGHT_PX}px`,
        "--base-row-line-clamp": lineClamp == null ? "none" : String(lineClamp),
      }) as React.CSSProperties,
    [dataRowHeightPx, lineClamp],
  );
  ...
  const rowVirtualIndexById = useMemo(() => {
    const map = new Map<string, number>();
    displayItems.forEach((item, index) => {
      if (item.kind === "row") map.set(item.id, index);
    });
    return map;
  }, [displayItems]);
  ...
  const estimateSize = useCallback(
    (index: number) => {
      const kind = displayItems[index]?.kind;
      if (kind === "group" || kind === "add-row") return COMPACT_ROW_HEIGHT_PX;
      return dataRowHeightPx;
    },
    [displayItems, dataRowHeightPx],
  );
  ...
  const prevDataRowHeightRef = useRef(dataRowHeightPx);
  useLayoutEffect(() => {
    if (prevDataRowHeightRef.current === dataRowHeightPx) return;
    prevDataRowHeightRef.current = dataRowHeightPx;
    virtualizer.measure();
  }, [dataRowHeightPx, virtualizer]);
  ...
  const scrollCellIntoView = useCallback(
    (coord: CellCoord, rowIndex: number) => {
      const virtualIndex = rowVirtualIndexById.get(coord.rowId) ?? rowIndex;
      if (virtualIndex >= 0) {
    ...
    [virtualizer, pinnedLeftWidth, rowVirtualIndexById],
  );

  useEffect(() => {
    if (!editingCell) return;
    const idx = rowVirtualIndexById.get(editingCell.rowId) ?? -1;
    if (idx >= 0) scrollCellIntoView(editingCell, idx);
  }, [editingCell, scrollCellIntoView, rowVirtualIndexById]);
  ...
  useEffect(() => {
    ...
    if (!lastItem) return;
    if (lastItem.index < displayItems.length - OVERSCAN * 2) return;
  ...
  const handleAddGroupedRow = useCallback(
    (afterRowId: string | undefined, cells: Record<string, unknown>) => {
      onAddRow?.(afterRowId, undefined, cells);
    },
    [onAddRow],
  );
                ...
                if (item.kind === "add-row") {
                  return (
                    <AddRowButton
                      ...
                      measureRef={virtualizer.measureElement}
                      onClick={() =>
                        handleAddGroupedRow(item.afterRowId, item.cells)
                      }
                    />
                  );
                }
