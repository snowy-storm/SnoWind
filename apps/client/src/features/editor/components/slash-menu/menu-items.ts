...
const CommandGroups: SlashMenuGroupedItemsType = {
  basic: [
    ...
    {
      title: "Heading N",
      description: "Choose a deeper heading level (4–9).",
      searchTerms: [
        "h4",
        "h5",
        "h6",
      ...
      children: ([4, 5, 6, 7, 8, 9] as const).map((level) => ({
        ...
        searchTerms: [`h${level}`, `heading ${level}`, `heading${level}`],
  ...
  return (
    ...
    (item.searchTerms &&
      item.searchTerms.some(
        (term: string) =>
          term.includes(search) || i18n.t(term).toLowerCase().includes(search),
      ))
  );
};
...
export const getSuggestionItems = ({
  ...
  for (const [group, items] of Object.entries(CommandGroups)) {
    const filteredItems: SlashMenuItemType[] = [];

    for (const item of items) {
      if (item.hidden) continue;
      if (excludeItems?.has(item.title)) continue;
      ...
      if (item.children?.length) {
        ...
        const matchingChildren = item.children.filter((child) =>
          matchesItem(child, search, fuzzyMatch),
        );
        if (matchingChildren.length) {
          filteredItems.push(...matchingChildren);
        } else if (matchesItem(item, search, fuzzyMatch)) {
          filteredItems.push(item);
        }
        continue;
      }

      if (matchesItem(item, search, fuzzyMatch)) {
        filteredItems.push(item);
      }
    }
