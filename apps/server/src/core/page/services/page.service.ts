...
import { getEmptyDrawingContent } from '../drawing-content';
...
} from '@snowind/db/pagination/cursor-pagination';
...
export class PageService {
  ...
  async create(
    ...
    } else if (createPageDto.drawingType && !createPageDto.isBase && !isBase) {
      const drawingContent = getEmptyDrawingContent(createPageDto.drawingType);
      jsonToNode(drawingContent);
      content = drawingContent;
      textContent = jsonToText(drawingContent);
      ydoc = createYdocFromJson(drawingContent);
    }
    ...
    const page = await this.pageRepo.insertPage({
      ...
      isBase: createPageDto.isBase ?? isBase,
      drawingType:
        createPageDto.isBase || isBase
          ? null
          : (createPageDto.drawingType ?? null),
      content,
      textContent,
      ydoc,
    }, trx);
