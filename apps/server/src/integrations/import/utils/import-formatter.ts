import { getEmbedUrlAndProvider } from '@snowind/editor-ext';
import { Logger } from '@nestjs/common';
import * as path from 'path';
import { v7 } from 'uuid';
import { InsertableBacklink } from '@snowind/db/types/entity.types';
import { Cheerio, CheerioAPI, load } from 'cheerio';
import slugify from '@sindresorhus/slugify';
import { normalizeTableColumnWidths } from './table-utils';
import { applyConfluenceMacros } from '../../../ee/confluence-import/confluence-html';
 ...
 * - confluenceFormatter: no-op on non-Confluence HTML (macro class-based).
...
/**
 * Lift block-level import nodes out of wrapping <p>/<a> tags so Tiptap
 * can parse them. Image is a block node, so <p><img></p> would otherwise
 * drop the image. Tables wrapped in paragraphs are also invalid HTML.
 */
...
export async function rewriteInternalLinksToMentionHtml(
  ...
  $root.find('a[href]').each((_, el) => {
    ...
    decodedRaw = decodedRaw.split('#')[0].split('?')[0];
