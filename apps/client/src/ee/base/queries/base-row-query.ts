...
function rowAlreadyInCache(
  data: InfiniteData<IBaseRowsPage> | undefined,
  rowId: string,
): boolean {
  return !!findRowInInfinite(data, rowId);
}

export function patchRowInCaches(
  pageId: string,
  rowId: string,
  updater: (row: IBaseRow) => IBaseRow,
): void {
  ...
  queryClient.setQueryData<IBaseRow>(["base-row", pageId, rowId], (old) =>
    old ? updater(old) : old,
  );
}
  ...
  return useMutation<IBaseRow, Error, CreateRowInput>({
    ...
    onSuccess: (newRow) => {
      queryClient.setQueriesData<InfiniteData<IBaseRowsPage>>(
        ...
        (old) => {
          if (!old) return old;
          if (rowAlreadyInCache(old, newRow.id)) return old;
  ...
  return useMutation<IBaseRow, Error, UpdateRowInput, RowCacheContext>({
    ...
    onMutate: async (variables) => {
      ...
      const applyOptimistic = () => {
        patchRowInCaches(variables.pageId, variables.rowId, (row) => ({
          ...row,
      ...
      // Patch first so the grid/kanban re-render before any in-flight list
      // request is cancelled; re-apply after cancel so a late response cannot
      // clobber the optimistic cells.
      applyOptimistic();
    ...
    onSuccess: (updatedRow, variables) => {
      patchRowInCaches(updatedRow.pageId, updatedRow.id, (row) => ({
        ...row,
        ...updatedRow,
      ...
      if (
        (base?.views ?? []).some((v) => v.type === "kanban") &&
        changedPropertyIds.some((id) => kanbanGroupByIds.has(id))
      ) {
...
export function useKanbanCreateCardMutation() {
  ...
  return useMutation<IBaseRow, Error, KanbanCreateCardInput>({
    mutationFn: ({ pageId, groupByPropertyId, columnKey, position, cells }) => {
      const nextCells: Record<string, unknown> = { ...(cells ?? {}) };
      if (columnKey !== NO_VALUE_CHOICE_ID && nextCells[groupByPropertyId] === undefined) {
        nextCells[groupByPropertyId] = columnKey;
      }
      return createRow({
        pageId,
        cells: nextCells,
...
export function invalidateBaseRows(pageId: string) {
  ...
  queryClient.invalidateQueries({ queryKey: ["base-row", pageId] });
}
