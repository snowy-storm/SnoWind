...
import { canEditBaseView } from "@/ee/base/utils/view-access";
...
export function ViewTabs({
  ...
  const handleReorder = useCallback(
    (sourceId: string, targetId: string, edge: Edge) => {
      ...
      const source = orderedViews.find((v) => v.id === sourceId);
  ...
  const handleTogglePrivate = useCallback(
    (view: IBaseView, isPrivate: boolean) => {
      if (view.isDefault || view.creatorId !== currentUserId) return;
      updateViewMutation.mutate({ viewId: view.id, pageId, isPrivate });
    },
    [currentUserId, pageId, updateViewMutation],
  );

  const handleDelete = useCallback(
    (viewId: string) => {
      const view = orderedViews.find((v) => v.id === viewId);
      if (!view || view.isDefault || orderedViews.length <= 1) return;
  ...
  return (
    <Group gap={4}>
      {orderedViews.map((view) => {
        const viewCanEdit = canEditBaseView(view, currentUserId, pageCanEdit);
        const isCreator = !!currentUserId && view.creatorId === currentUserId;
        return (
        <ViewTab
          key={view.id}
          view={view}
          ...
          canDelete={viewCanEdit && !view.isDefault && orderedViews.length > 1}
          canRename={viewCanEdit}
          canTogglePrivate={viewCanEdit && isCreator && !view.isDefault}
          reorderEnabled={
            pageCanEdit && !view.isDefault && orderedViews.length > 1
          }
          dropEnabled={pageCanEdit && orderedViews.length > 1}
          ...
          onTogglePrivate={(isPrivate) => handleTogglePrivate(view, isPrivate)}
          getViewShareUrl={view.isPrivate ? undefined : getViewShareUrl}
        />
        );
      })}
      {canAddView && base && (
...
function ViewTab({
  ...
  useEffect(() => {
    ...
    return combine(...listeners);
  }, [view.id, reorderEnabled, dropEnabled, isEditing]);
          ...
          {canTogglePrivate && (
            ...
            >
              <Group gap={8} wrap="nowrap">
                ...
                <Text size="sm">
                  {view.isPrivate ? t("Private") : t("Public")}
                </Text>
              </Group>
              <Switch
                size="xs"
                checked={!view.isPrivate}
                onChange={(e) =>
                  onTogglePrivate(!e.currentTarget.checked)
                }
                aria-label={t("Public")}
              />
            </Group>
