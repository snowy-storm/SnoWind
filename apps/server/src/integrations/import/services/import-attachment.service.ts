...
import { v7 } from 'uuid';
import { FileTask } from '@snowind/db/types/entity.types';
...
export class ImportAttachmentService {
  ...
  async processAttachments(opts: {
    ...
    const pageDir = path.dirname(pageRelativePath).split(path.sep).join('/');
    const byBasename = buildAttachmentBasenameIndex(attachmentCandidates);
    const resolveLocal = (raw: string, attachmentId?: string) =>
      resolveRelativeAttachmentPath(raw, pageDir, attachmentCandidates, {
        attachmentId,
        byBasename,
      });
    ...
    for (const attachment of pageAttachments) {
      const relPath = resolveLocal(attachment.href);
    ...
    const replaceWithDrawing = (
      ...
      const $drawing = createDrawingNode(type, opts);
      $el.replaceWith($drawing);
      unwrapFromParagraph($, $drawing);
    };

    // Rewrite existing drawing nodes first so nested preview <img> tags
    // are not turned into standalone images.
    for (const type of ['excalidraw', 'drawio'] as const) {
      for (const el of $(`div[data-type="${type}"]`).toArray()) {
        ...
        if (!rawSrc || isRemoteOrEmbeddedUrl(rawSrc)) continue;

        const relPath = resolveLocal(
          rawSrc,
          $oldDiv.attr('data-attachment-id'),
        );
        if (!relPath) continue;
        ...
        const $newDiv = createDrawingNode(type, {
          apiFilePath,
          attachmentId,
          title: fileName,
          width,
    ...
    for (const imgEl of $('img').toArray()) {
      const $img = $(imgEl);
      if (
        $img.closest('div[data-type="drawio"], div[data-type="excalidraw"]')
          .length
      ) {
        continue;
      }
      ...
      if (!src || isRemoteOrEmbeddedUrl(src)) continue;

      const relPath = resolveLocal(src, $img.attr('data-attachment-id'));
      if (!relPath) {
        this.logger.debug(
          `Could not resolve image path "${src}" on ${pageRelativePath}`,
        );
        continue;
      }
      ...
      if (drawioSvg) {
        replaceWithDrawing($img, 'drawio', {
          apiFilePath: drawioSvg.apiFilePath,
          attachmentId: drawioSvg.attachmentId,
        });
        continue;
      }

      const { attachmentId, apiFilePath, abs, fileNameWithExt } =
        processFile(relPath);
      const drawingType = getDrawingNodeType(fileNameWithExt);
      if (drawingType) {
        replaceWithDrawing($img, drawingType, {
          apiFilePath,
          attachmentId,
          title: fileNameWithExt,
          width: $img.attr('width') || $img.attr('data-width') || '100%',
          align: $img.attr('data-align') || 'center',
        });
        continue;
      }
    ...
    for (const vidEl of $('video').toArray()) {
      const $vid = $(vidEl);
      const src =
        cleanUrlString($vid.attr('src') ?? '') ||
        cleanUrlString($vid.find('source').first().attr('src') ?? '');
      if (!src || isRemoteOrEmbeddedUrl(src)) continue;

      const relPath = resolveLocal(src, $vid.attr('data-attachment-id'));
      if (!relPath) continue;
    ...
    for (const audEl of $('audio').toArray()) {
      const $aud = $(audEl);
      const src =
        cleanUrlString($aud.attr('src') ?? '') ||
        cleanUrlString($aud.find('source').first().attr('src') ?? '');
      if (!src || isRemoteOrEmbeddedUrl(src)) continue;

      const relPath = resolveLocal(src, $aud.attr('data-attachment-id'));
      if (!relPath) continue;
    ...
    for (const el of $('div[data-type="attachment"]').toArray()) {
      ...
      if (!rawUrl || isRemoteOrEmbeddedUrl(rawUrl)) continue;
      ...
      const drawingType = getDrawingNodeType(fileNameWithExt);
      if (drawingType) {
        replaceWithDrawing($oldDiv, drawingType, {
          apiFilePath,
          attachmentId,
          title: fileNameWithExt,
        });
    ...
    for (const aEl of $('a').toArray()) {
      ...
      if (!href || isRemoteOrEmbeddedUrl(href)) continue;

      const relPath = resolveLocal(href, $a.attr('data-attachment-id'));
      if (!relPath) continue;

      const drawioSvg = drawioSvgMap.get(relPath);
      if (drawioSvg) {
        replaceWithDrawing($a, 'drawio', {
          apiFilePath: drawioSvg.apiFilePath,
          attachmentId: drawioSvg.attachmentId,
        });
        continue;
      ...
      if (skipFiles.has(relPath) || skipFiles.has(href)) {
        $a.remove();
        continue;
      }
      ...
      const drawingType = getDrawingNodeType(fileNameWithExt);
      if (drawingType) {
        replaceWithDrawing($a, drawingType, {
          apiFilePath,
          attachmentId,
          title: fileNameWithExt,
        });
    ...
    // Second pass: rewrite any leftover local refs that the typed
    // handlers missed (srcset-less data-src, nested attrs, etc.).
    const leftoverAttrs = [
      'src',
      'href',
      'data-src',
    ...
    for (const el of $('*').toArray()) {
      ...
      for (const attr of leftoverAttrs) {
        const raw = cleanUrlString($el.attr(attr) ?? '');
        if (!raw || isRemoteOrEmbeddedUrl(raw)) continue;
        if (raw.startsWith('/api/files/')) continue;

        const relPath = resolveLocal(raw, $el.attr('data-attachment-id'));
        if (!relPath) continue;

        const { attachmentId, apiFilePath } = processFile(relPath);
        $el.attr(attr, apiFilePath);
        if (!$el.attr('data-attachment-id')) {
          $el.attr('data-attachment-id', attachmentId);
        }
      }
    }
    ...
    for (const [drawioHref] of drawioPairs) {
      ...
      const $drawio = createDrawingNode('drawio', {
        apiFilePath: drawioSvg.apiFilePath,
        attachmentId: drawioSvg.attachmentId,
        width: '600',
      });
    ...
    // Store co-located images and drawings that the page did not reference.
    for (const relPath of collectCoLocatedMediaPaths(
      pageRelativePath,
      attachmentCandidates,
    )) {
      if (processed.has(relPath) || skipFiles.has(relPath)) {
        continue;
      }
      const absPath = attachmentCandidates.get(relPath);
      ...
      if (drawingType) {
      ...
      } else if (isImageFileName(fileNameWithExt)) {
        const $img = $('<img>')
          .attr('src', apiFilePath)
          .attr('data-attachment-id', attachmentId)
          .attr('data-align', 'center');
        ...
        const $attachmentDiv = $('<div>')
          .attr('data-type', 'attachment')
          .attr('data-attachment-url', apiFilePath)
          .attr('data-attachment-name', path.basename(abs))
          .attr('data-attachment-mime', getMimeType(abs))
          .attr('data-attachment-id', attachmentId);
    ...
    for (const attachment of pageAttachments) {
      ...
      const resolvedHref = resolveLocal(href);
