...
export class FileImportTaskService {
  ...
  async processGenericImport(opts: {
    ...
    const snowindMetadata = await readSnoWindMetadata(extractDir);
    ...
    for (const absPath of allFiles) {
      ...
      const pageMetadata = snowindMetadata?.pages[encodedPath];
    ...
    sortedFolders.forEach((folderPath) => {
      ...
      if (!pagesMap.has(mdPath) && !pagesMap.has(htmlPath)) {
        ...
        if (!matched) {
          ...
          const placeholderMetadata = snowindMetadata?.pages[encodedMdPath];
          pagesMap.set(mdPath, {
            id: v7(),
            slugId: generateSlugId(),
    ...
    const sortSiblings = (siblings: ImportPageNode[]) => {
      if (snowindMetadata) {
        siblings.sort((a, b) => {
          const posA =
            snowindMetadata.pages[encodedPathsMap.get(a.filePath)]?.position;
          const posB =
            snowindMetadata.pages[encodedPathsMap.get(b.filePath)]?.position;
          if (posA && posB) {
