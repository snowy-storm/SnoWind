...
import { invalidateBaseRows, patchRowInCaches } from "@/ee/base/queries/base-row-query";
...
export function useBaseSocket(pageId: string | undefined): void {
  ...
  useEffect(() => {
    ...
    const handler = (raw: unknown) => {
      ...
      const eventPageId =
        event.pageId ||
        (event as { baseId?: string }).baseId;
      if (eventPageId !== pageId) return;
      ...
      switch (event.operation) {
        ...
        case "base:row:updated": {
          ...
          const rowId = e.rowId ?? e.row?.id;
          if (!rowId) break;
          patchRowInCaches(pageId, rowId, (row) => {
            if (e.row) {
              return {
                ...row,
                ...e.row,
                cells: { ...row.cells, ...(e.row.cells ?? {}), ...(e.updatedCells ?? {}) },
              };
            }
            return {
              ...row,
          ...
          if (e.row) {
            queryClient.setQueryData<IBaseRow>(
              ["base-row", pageId, rowId],
              (old) =>
                old
                  ? { ...old, ...e.row, cells: { ...old.cells, ...e.row!.cells } }
                  : e.row,
            );
          }
