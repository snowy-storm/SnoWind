...
import { computeOutlineLabels } from "@snowind/editor-ext";
...
const recalculateLinks = (nodePos: NodePos[] | false | null) => {
  const nodes: HTMLElement[] = [];
  const items = nodePos ? Array.from(nodePos) : [];
  const outlines = computeOutlineLabels(
    items.map((item) => Number(item.node.attrs.level) || 1),
  );

  const links: HeadingLink[] = items.reduce<HeadingLink[]>(
    (acc, item, index) => {
      const label = item.node.textContent;
      const level = Number(item.node.attrs.level);
      if (label.length && level <= 9) {
        acc.push({
          label,
          outline: item.node.attrs.numbered ? outlines[index] : null,
          level,
  ...
  if (!links.length) {
    return (
      <>
        {!props.isShare && (
          <Text size="sm">
            {t("Add headings (H1–H9) to generate a table of contents.")}
          </Text>
        )}
      ...
      <div className={props.isShare ? classes.leftBorder : ""}>
        {links.map((item, idx) => (
          ...
          >
            {item.outline ? (
              <>
                <span className={classes.outline}>{item.outline}</span>
                {item.label}
              </>
            ) : (
              item.label
