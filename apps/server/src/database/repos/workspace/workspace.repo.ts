...
import { DB, Workspaces } from '@snowind/db/types/db';
...
export class WorkspaceRepo {
  ...
  async updatePageVerificationSettings(
    workspaceId: string,
    prefKey: string,
    prefValue: string | boolean,
    trx?: KyselyTransaction,
    ...
    return db
      ...
      .set({
        settings: sql`COALESCE(settings, '{}'::jsonb)
                || jsonb_build_object('pageVerification', COALESCE(settings->'pageVerification', '{}'::jsonb)
                || jsonb_build_object('${sql.raw(prefKey)}', ${sql.lit(prefValue)}))`,
        updatedAt: new Date(),
      })
      .where('id', '=', workspaceId)
