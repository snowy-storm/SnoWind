...
import { ViewGroupConfigPopover } from "@/ee/base/components/views/view-group-config";
...
import { ViewRowHeightPicker } from "@/ee/base/components/views/view-row-height";
...
type BaseToolbarProps = {
  ...
  onDraftGroupsChange: (groups: ViewGroupConfig[] | undefined) => void;
  onExpand?: () => void;
...
export function BaseToolbar({
  ...
  const isTable = (activeView?.type ?? "table") === "table";
  ...
  }, [base.id, exporting, t, activeView?.config?.filter]);

  const openToolbar = useCallback((panel: "sort" | "filter" | "group" | "properties") => {
    setSortOpened(panel === "sort" ? (v) => !v : false);
    setFilterOpened(panel === "filter" ? (v) => !v : false);
    setGroupOpened(panel === "group" ? (v) => !v : false);
    setPropertiesOpened(panel === "properties" ? (v) => !v : false);
  }, []);

  const sorts = activeView?.config?.sorts ?? [];
  const groups = activeView?.config?.groups ?? [];
  ...
  const handleGroupsChange = useCallback(
    (newGroups: ViewGroupConfig[]) => {
      onDraftGroupsChange(newGroups.length > 0 ? newGroups : undefined);
    },
    [onDraftGroupsChange],
  );
          ...
          >
            <Tooltip label={t("Group by")}>
              <ActionIcon
                ...
                size="sm"
                pos="relative"
                color={groups.length > 0 ? "blue" : "gray"}
                onClick={() => openToolbar("group")}
              >
