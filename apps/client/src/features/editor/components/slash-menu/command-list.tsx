...
const CommandList = ({
  ...
  const flatItems = useMemo(() => {
    const flat: SlashMenuItemType[] = [];
    for (const categoryItems of Object.values(items)) {
      for (const item of categoryItems) {
        flat.push(item);
        if (item.children?.length && expandedTitles.has(item.title)) {
          flat.push(...item.children);
        }
      }
    }
    return flat;
  }, [items, expandedTitles]);
        ...
        {(() => {
          ...
          <div key={category} role="group" aria-label={category}>
            ...
            {categoryItems.map((item: SlashMenuItemType) => {
              const renderRow = (row: SlashMenuItemType, nested: boolean) => {
                flatIndex += 1;
                const itemIndex = flatIndex;
                const disabled = isItemDisabled(row);
                const expanded = Boolean(
                  row.children?.length && expandedTitles.has(row.title),
                );
                return (
              <Tooltip
                key={`${row.title}-${itemIndex}`}
                label={upgradeLabel}
                disabled={!disabled}
                position="right"
              >
