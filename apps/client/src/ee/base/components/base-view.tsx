...
export function BaseView({ pageId, embedded, editable = true, titleSlot }: BaseViewProps) {
  ...
  const views = useMemo(
    () =>
      sortBaseViews(
        (base?.views ?? []).filter((view) =>
          isBaseViewVisibleToUser(view, currentUserId),
        ),
      ),
    [base?.views, currentUserId],
  );
  ...
  const guardedPersistViewConfig = useCallback(() => {
    ...
    persistViewConfig();
  }, [viewEditable, persistViewConfig]);
  ...
  const handleAddRow = useCallback(
    ...
    [viewEditable, pageId, createRowMutation, table, setEditingCell, setFocusedCell],
  );
  ...
  const handleDraftGroupsChange = useCallback(
    (groups: ViewGroupConfig[] | undefined) => {
      setDraftGroups(groups && groups.length > 0 ? groups : undefined);
    },
    [setDraftGroups],
  );
  ...
  const handleSaveDraft = useCallback(async () => {
    ...
    // Preserves non-draft baseline fields (widths/order/visibility), overwrites only filter/sorts/groups.
  ...
  const handleRowReorder = useCallback(
    ...
    [viewEditable, pageId, reorderRow],
  );
  ...
  const toolbar = (
    <BaseToolbar
      ...
      onDraftGroupsChange={handleDraftGroupsChange}
  ...
  const viewRenderer = (folded: React.ReactNode) => (
    <ViewRenderer
      ...
      onColumnReorder={viewEditable ? handleColumnReorder : undefined}
      onResizeEnd={handleResizeEnd}
      onRowReorder={viewEditable ? handleRowReorder : undefined}
  ...
  if (embedded) {
    if (isKanban) {
      return (
        <BaseEditableProvider editable={viewEditable}>
